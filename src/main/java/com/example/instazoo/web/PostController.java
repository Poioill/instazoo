...
public class PostController {
    ...
    public ResponseEntity<List<PostDTO>> getAllPosts(){
        List<PostDTO> postDTOSList = postService.getAllPosts()
                .stream()
                .map(postFacade::postToPostDTO)
                .collect(Collectors.toList());

        return new ResponseEntity<>(postDTOSList, HttpStatus.OK);
    }

    @GetMapping("/user/posts")
    public ResponseEntity<List<PostDTO>> getAllPostsForUser(Principal principal){
        List<PostDTO> postDTOSList = postService.getAllPostForUser(principal)
                .stream()
                .map(postFacade::postToPostDTO)
                .collect(Collectors.toList());
    ...
    public ResponseEntity<PostDTO> likePost(@PathVariable("postdId") String postId,
